        ...
        foreach ((array)$relaciones as $relacion) {
           ...
           // $rowData .= "<td>{$relacion->getId()}</td>";
            ...
            if ($tipoRelacion->getId() == $relacion->getTipoRelacionA()) {
                    $rowData .= "<td>{$tipoRelacion->getNombre()}</td>";
                    break;
                }
            }
...
*/
            ...
            $rowData .= "<td><a href='/editar_parentesco?id={$relacion->getId()}' class='edit-button'>Editar</a></td>";
            $rowData .= "<td>";
            $rowData .= "<form id='form_boton_eliminar' method='post' action='/eliminar_parentesco'>";
    ...
    private function renderizarFormularioCreacion($usuarios, $tiposRelaciones): string
    {
        ...
        $formulario .= "<h2>Nuevo Parentesco</h2>";
        $formulario .= "</div>";
        $formulario .= "<div class='container'>";
        $formulario .= "<form method='post' action='/parentescos'>";
    ...
    public function actualizar($relaciones, $usuarios, $tiposRelaciones): void
    {
        ...
        include '../app/vistas/Parentesco/index.php';
    }
    ...
    private function renderizarFormularioEdicion($relacion, $usuarios, $tiposRelaciones): string
    {
        ...
        if (!empty($relacion) && $relacion instanceof Parentesco) {
            $formulario .= "<form method='post' action='/actualizar_parentesco'>";
          ...
          //  $formulario .= "</select>";
            ...
            $formulario .= "<a href='/parentescos' class='back-button'>Volver</a>";
            $formulario .= "</div>";
            $formulario .= "</div>";
            $formulario .= "</form>";
        } else {
